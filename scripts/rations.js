async function rationTrackingInit() {
...
async function checkRations(arr) {
  for (let id of arr) {
...
*/
async function oseActorReport(data) {
  console.log(data);
  const actor = game.actors.getName(data.actorName);
  const Rations = [];
  const Lights = [];

  for (let key in oseFood) {
    Rations.push(key);
  }
  for (let key in oseLight) {
    Lights.push(key);
  }
  let totalRations = 0;
  const msgData = {
  ...
  for (let name of Rations) {
    ...
    let ration = actor.data.items.getName(oseFood[name]);
    console.log(ration);
    if (ration) {
      ...
      actorItem += `<li><span style="${style}">${oseFood[name]}: ${qty}</span></li>`;
      msgData.food += `<div><p> ${oseFood[name]}:</p><ul>` + actorItem + `</ul></div>`;
    }
  }
  for (let name of Lights) {
    ...
    let light = actor.data.items.getName(oseLight[name].name);
    if (light) {
      ...
      if (qty <= 2) style = 'color: orangered';
      if (qty <= 1) style = 'color: red';
      actorItem += `<li><span style="${style}">${oseLight[name].name}: ${light.data.data.quantity.value}</span></li>`;
      msgData.light += `<div><p> ${oseLight[name].name}:</p><ul>` + actorItem + `</ul></div>`;
    }
  }
...
async function oseRationReport(data) {
  const Rations = [];
  for (let key in oseFood) {
    Rations.push(oseFood[key]);
  }
  //console.log('rat', Rations);
  let totalRations = 0;
...
async function oseEat(actorId) {
  const actor = await game.actors.find((a) => a.id == actorId);
  let rationOptions = '';
  for (let type in oseFood) {
    const item = actor.data.items.getName(oseFood[type]);
    if (item) {
