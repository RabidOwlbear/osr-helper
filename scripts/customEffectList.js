...
class customEffectList extends Application {
  constructor(actorId = null, user) {
    super();
    this.user = user;
    if (!actorId == null) {
      this.actorId = actorId;
      this.actor = oseGetActorById(actorId);
  ...
  async addEffect(html) {
    ...
    oseHook('OSE-helper newEffectBtnToggle', [newEffectBtn]);
    const formData = {
      _id: randomID(16),
...
async function oseEffectList(actor = null, user = game.user) {
  const id = actor == null ? null : actor.id;
  let list = new customEffectList(id, user);
  list.shop = list;
  list.render(true);
    ...
    buttons: {
      getList: {
        ...
        callback: (html) => {
          ...
          oseEffectList(null, game.users.get(selected));
        }
      }
    }
  }).render(true);
